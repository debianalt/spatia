<script lang="ts">
	...
	const ANALYSIS_CONTENT: Record<string, { howToRead: string; implications: string; method: string }> = {
		...
		pm25_drivers: {
			...
			method: 'Descomposicion por machine learning (LightGBM, SHAP feature attribution) de la concentracion media anual de PM2.5. Fuente primaria: Atmospheric Composition Analysis Group (ACAG) V6.GL.02 (Dalhousie University, van Donkelaar et al. 2021), panel 1998-2022, resolucion 0.01 deg (~1 km). Modelo entrenado con 31 covariables ambientales (R2 = 0.93 en validacion cruzada espacial leave-one-department-out). Drivers agrupados por SHAP: fuego regional (contribucion dominante, dR2 = 0.195), clima (precipitacion, temperatura), terreno (elevacion, pendiente) y vegetacion (NDVI, NPP). El toggle temporal compara periodo 2001-2010 vs 2013-2022. Resolucion espacial: H3 resolucion 9.',
		},
		productive_activity: {
			howToRead: 'El mapa muestra la intensidad de actividad productiva medida por luces nocturnas satelitales (VIIRS). Colores calidos = mayor radiancia nocturna = mayor actividad economica. Al hacer click se ven 6 indicadores en valores reales: luces, productividad vegetal, verdor, superficie construida, conversion forestal y temperatura. El toggle temporal compara con el periodo base (2014-2017 para VIIRS, 2005-2012 para otros indicadores).',
			implications: 'Las zonas con alta radiancia nocturna y crecimiento positivo (delta > 0) son polos economicos en expansion. Zonas con alta productividad vegetal (NPP) pero baja radiancia son areas rurales productivas pero no urbanizadas. Un aumento de temperatura superficial (LST) junto con aumento de superficie construida indica urbanizacion activa. La conversion forestal alta combinada con baja actividad economica puede indicar deforestacion sin desarrollo productivo asociado.',
			method: 'Seis indicadores satelitales en valores fisicos reales (sin scores compuestos ni indices artificiales). VIIRS nightlights (NOAA, 500m, 2014-2025): radiancia media nocturna en nW/cm2/sr. NPP (MODIS, 1km, 2005-2024): productividad primaria neta en gC/m2/ano. NDVI (MODIS, 250m, 2005-2024): indice de vegetacion normalizado. GHSL built surface (JRC, 10m, epochs 2000/2020): fraccion de superficie construida. Hansen forest loss (UMD/Landsat, 30m, 2001-2024): perdida acumulada. LST (MODIS, 1km, 2005-2024): temperatura superficial diurna en grados Celsius. Cada hexagono H3 res-9 recibe el valor ponderado por la fraccion de area del radio censal que lo cubre (crosswalk dasimetrico areal). El color del mapa representa el percentil provincial de la radiancia nocturna.',
		},
		deforestation_dynamics: {
			howToRead: 'Cada hexagono muestra la tasa de perdida forestal observada en ese punto exacto (pixel Landsat 30m). Colores calidos = mayor perdida forestal reciente (2015-2024). El toggle temporal permite comparar con la linea base (2001-2010): el modo "Cambio" muestra si la deforestacion acelero (rojo) o freno (verde) respecto al periodo base.',
			implications: 'Las zonas con alta tasa de perdida sostenida representan frentes de deforestacion activos donde la conversion del bosque nativo continua. Las zonas donde la deforestacion freno (delta negativo) pueden reflejar el efecto de la Ley de Bosques (26.331/OTBN, vigente desde 2007) o el agotamiento del recurso. Las zonas que aceleraron post-2015 a pesar de la regulacion requieren atencion prioritaria de fiscalizacion.',
			method: 'Fuente: Hansen Global Forest Change v1.12 (University of Maryland / Google), derivado de series temporales Landsat a 30m de resolucion, cobertura 2001-2024. Cada hexagono H3 resolucion 9 (~0.1 km2) recibe el valor del pixel de perdida en su centroide — no hay promedio por radio censal. La tasa de perdida se calcula como fraccion de anos con perdida detectada en cada periodo. Linea base: 2001-2010; actual: 2015-2024. Actualizacion automatica via GitHub Actions cuando Hansen publica nuevos datos (~abril de cada ano).',
		},
	};
