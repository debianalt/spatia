...
export const ANALYSIS_CONTENT: Record<string, MethodologyContent> = {
	...
	forestry_aptitude: {
		...
		method: `Random Forest (sklearn, 400 árboles, class_weight balanced) entrenado como SDM presencia-background con datos de Misiones: presencia = hexágonos MapBiomas Argentina clase silvicultura ≥50% (1,648 hexes); background = muestra aleatoria (ratio 1:3). Covariables biofísicas a resolución H3: clima (ERA5 GDD, temperatura y radiación; CHIRPS precipitación y eventos extremos; TerraClimate déficit hídrico, VPD, humedad del suelo), suelo (SoilGrids arcilla/limo/arena/pH/SOC; HWSD drenaje, textura, profundidad radicular, AWC), terreno (FABDEM pendiente, elevación, TWI, rugosidad), vegetación contextual (NDVI medio) y accesibilidad (tiempo a ciudad 50k). Heladas excluidas: en Misiones son 0-2 días/año, aportan ruido sin señal. Validación: Group K-Fold espacial por parent H3 res 5, AUC=0.77. Aplicación a otros territorios: extrapolación con las mismas covariables satelitales globales (no requiere presencia local). Expansión al grid completo (~320k hexes) vía interpolación KNN ponderada por distancia inversa (k=5). Tipos: k-means k=4 sobre covariables de alto score.`,
	},
	service_deprivation: {
		howToRead:
