...
Territory-aware: reads MapBiomas classification raster and computes per-class fractions
for each H3 hexagon, then runs PCA + k-means typology.

Raster resolution:
  - Misiones/AR territories: MapBiomas AR Collection 1 (30m), exported by gee_export_mapbiomas.py
  - Itapúa (legacy): sdm_mapbiomas_py.tif from gee_export_sdm_covariates.py

Usage:
  python pipeline/process_mapbiomas_to_h3.py
  python pipeline/process_mapbiomas_to_h3.py --territory corrientes
  python pipeline/process_mapbiomas_to_h3.py --territory itapua_py
  python pipeline/process_mapbiomas_to_h3.py --input /path/to/mapbiomas_corrientes_2023.tif --territory corrientes
"""
...
def main():
    ...
    parser.add_argument('--input', default=None, help='Input GeoTIFF path (overrides auto-detection)')
    ...
    if args.territory != 'misiones':
        # Resolve raster path for non-Misiones territories
        if args.input:
            mapbiomas_raster = args.input
        else:
            # Look for MapBiomas AR raster exported by gee_export_mapbiomas.py
            candidates = sorted(glob.glob(os.path.join(t_dir, f'mapbiomas_{args.territory}_*.tif')))
            if candidates:
                mapbiomas_raster = candidates[-1]  # most recent year
                print(f"Found MapBiomas raster: {os.path.basename(mapbiomas_raster)}")
            else:
                # Fallback: itapua_py legacy raster from SDM covariates pipeline
                mapbiomas_raster = os.path.join(t_dir, 'sdm_mapbiomas_py.tif')

        if not os.path.exists(mapbiomas_raster):
            print(f"ERROR: MapBiomas raster not found: {mapbiomas_raster}")
            print(f"  Run: python pipeline/gee_export_mapbiomas.py --territory {args.territory} --gcs")
            print(f"  Then download to: {t_dir}/")
            return
        ...
        if args.input:
            ...
            candidates = sorted([f for f in os.listdir(OUTPUT_DIR) if f.startswith('mapbiomas_misiones') and f.endswith('.tif')])
            if not candidates:
                print("No MapBiomas raster found. Run gee_export_mapbiomas.py first.")
                return
            raster_path = os.path.join(OUTPUT_DIR, candidates[-1])
