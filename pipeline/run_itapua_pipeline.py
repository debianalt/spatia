...
def step_gee_export(analyses: list[str], territory_id: str, dry_run: bool) -> bool:
...
def step_download_gcs(analyses: list[str], gcs_prefix: str, out_dir: str, dry_run: bool) -> bool:
...
def step_process_h3(analyses: list[str], territory_id: str, mode: str, dry_run: bool) -> bool:
    """Run process_raster_to_h3 for each analysis."""
    print("\n" + "=" * 60)
    print(f"  STEP 4: Raster -> H3 parquets (mode={mode})")
    print("=" * 60)
    ids = ",".join(analyses)
    rc = run(
        f"python pipeline/process_raster_to_h3.py "
        f"--territory {territory_id} --analysis {ids} --mode {mode}",
        dry_run
    )
    return rc == 0


def step_split_admin(analyses: list[str], territory_id: str, dry_run: bool) -> bool:
...
def step_upload_r2(analyses: list[str], r2_prefix: str, out_dir: str, dry_run: bool) -> bool:
    """Upload parquets to R2 under data/<output_prefix> prefix."""
    print("\n" + "=" * 60)
    ...
    print(f"\n  All uploads complete. R2 path: {r2_prefix}/")
    print(f"  Next: set available: true for this territory in src/lib/config.ts")
    return True
...
def main():
    ...
    # MUST include 'data/' — public bucket serves from pub-...r2.dev/data/.
    # Without it the object is orphaned and the browser reads a stale version.
    r2_prefix = f"data/{t_prefix.rstrip('/')}" if t_prefix else "data"
    ...
    print("=" * 60)
    print(f"  {territory['label']} Pipeline ({territory_id}) — {len(analyses)} analyses")
    ...
    if not os.path.exists(crosswalk_path) or not os.path.exists(hexgrid_path):
        ...
        print("  Run first:")
        print(f"    python pipeline/explore_admin.py --territory {territory_id}")
        print(f"    python pipeline/build_admin_crosswalk.py --territory {territory_id} --source gaul")
        if not args.dry_run:
            return 1
    ...
    if not args.skip_gee:
        if not step_gee_export(analyses, territory_id, args.dry_run):
            return 1

    if not args.skip_download:
        if not step_download_gcs(analyses, gcs_prefix, out_dir, args.dry_run):
            return 1

    if not step_process_h3(analyses, territory_id, args.mode, args.dry_run):
        return 1

    if not step_split_admin(analyses, territory_id, args.dry_run):
        return 1

    if args.upload:
        if not step_upload_r2(analyses, r2_prefix, out_dir, args.dry_run):
            return 1
